...
def test_resolve_backend_report_root_prefers_core_app(tmp_path):
    ...
    base_root = backend_root / "albunmania_app"
...
def test_resolve_backend_report_root_fallback_to_albunmania_app(tmp_path):
    backend_root = tmp_path / "backend"
    base_root = backend_root / "albunmania_app"
    base_root.mkdir(parents=True)
...
def _build_backend_summary_lines(tmp_path, monkeypatch):
    ...
    sample_path = backend_root / "albunmania_app" / "sample.py"
...
def test_run_backend_triggers_erase_when_enabled(tmp_path, monkeypatch):
    ...
    assert f"--cov={tmp_path / 'albunmania_app'}" in captured["command"]
...
def test_extract_backend_report_table_reads_header(tmp_path):
    ...
    log_path.write_text(
        "\n".join(
            [
                ...
                "albunmania_app/sample.py  1  0  100%  ██████",
                "===================== 24 passed in 2.0s =====================",
                "tail line",
            ]
        ),
    ...
    assert "albunmania_app/sample.py" in lines[2]
