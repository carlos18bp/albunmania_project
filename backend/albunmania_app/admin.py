...
class AlbunmaniaUserAdmin(UserAdmin):
    ...
    def login_as_link(self, obj):
        if not obj or not obj.pk:
            return '—'
        url = reverse('myadmin:albunmania_app_user_login_as', args=[obj.pk])
        return format_html(
    ...
    def get_urls(self):
        ...
        custom_urls = [
            path(
                ...
                name='albunmania_app_user_login_as',
            ),
        ]
    ...
    def login_as_user_view(self, request, user_id):
        ...
        change_url = reverse('myadmin:albunmania_app_user_change', args=[user_id])
...
class AlbunmaniaAdminSite(admin.AdminSite):
    site_header = 'Albunmanía Administration'
    site_title = 'Albunmanía Admin'
    index_title = 'Welcome to Albunmanía Control Panel'

    def get_app_list(self, request):
        app_dict = self._build_app_dict(request)
        base_app_models = app_dict.get('albunmania_app', {}).get('models', [])
