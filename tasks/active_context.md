...
- ✅ **F2** (`e101db3`): podadas las rutas vestigiales del template + el auth email/password muerto. **Frontend**: borradas `app/backoffice/` + `app/forgot-password/` (+ tests); quitados `ROUTES.BACKOFFICE/FORGOT_PASSWORD` + 6 de `API_ENDPOINTS` + la clave i18n `forgotPassword`; `authStore` perdió `signIn`/`signUp`/`sendPasswordResetCode`/`resetPassword`; fix de un test de auth E2E ambiguo (`Manual` link → scoped a `site-header`). **Backend**: `views/auth.py` ahora solo `google_login` + `validate_token`; eliminados `views/user_crud.py`, `serializers/user_{create_update,detail,list}.py`, `services/email_service.py`, `urls/user.py`, `models/password_code.py` (+ migración `0012_delete_passwordcode`); `auth_utils.py` → solo `generate_auth_tokens`; `admin.py`/`services/__init__.py`/`models/__init__.py` limpiados; tests muertos eliminados. Conteos: views 22→21, urls 22→21, serializers 14→11, services 15→14, models 20→19, migrations 11→12, ~73→~66 paths.
- ✅ **F3** (`df87ae8`): push de match → tarea Huey (`@db_task() deliver_match_push` en `albunmania_project/tasks.py`; el signal lo encola en vez de llamar `push_notify.send_to` síncrono; en dev/test Huey es `immediate` así que sigue síncrono). Tests: `test_tasks.py` (3) + el test del signal parchea `push_notify.send_to`.
- ✅ **F4** (`d799142`): filtros "disponibilidad" + "radio de proximidad" en `/catalog/[slug]`. **Backend**: `sticker_list` añade `availability=mine|missing|repeated` (estado del inventario del solicitante; anónimo → 400) y `nearby=true&lat&lng&radius_km` (cromos que algún colector dentro del radio tiene disponible — `count≥2`, excluye al solicitante; fallback a la location del `Profile`; 400 si no hay geo) — helper `_nearby_offerer_user_ids` (bbox+haversine sobre `Profile`, reusa `match_engine`). **Frontend**: `StickerFilters` ampliado; `CatalogFilters` con `<select>` de disponibilidad + checkbox "Disponibles cerca" (+ radio 10/25/50/100 km, deshabilitado sin location, con hint); `/catalog/[slug]` pasa `userLocation` (`refreshProfile()` al montar) + `catalog-error`. Flow `catalog-availability-proximity` (flow-definitions v2.9.0). Tests: backend `test_sticker_filters.py` (13), frontend `CatalogFilters`(8)+`albumStore`(+1), e2e `availability-proximity.spec.ts` (2).
- ✅ **F5** (commit pendiente): cobertura backend. La global ya estaba en **~93%** (la subieron los tests de Bloques D/E/F desde el ~57% histórico) — los módulos más bajos están en 86–90%. **1 tanda** (13 tests, no 2 — diminishing returns) cubriendo ramas de error de `views/collectors.py`, `views/album.py` (`nearby` con geo no numérico), `services/qr_token.py` (tokens forjados con payload inválido), `services/whatsapp_link.py` (trade unidireccional), `services/google_account_age.py` (createTime malformado / body no-JSON / earliest-de-varias-sources).
