...
- ✅ F4 (commit `d799142`): filtros "disponibilidad" + "radio de proximidad" en `/catalog/[slug]`. **Backend**: `sticker_list` añade `availability=mine|missing|repeated` (estado del inventario del solicitante; anónimo → 400) y `nearby=true&lat&lng&radius_km` (cromos que algún colector dentro del radio tiene disponible — `UserSticker.count≥2`, excluye al solicitante; fallback a la location del `Profile`; 400 si no hay geo) — helper `_nearby_offerer_user_ids` (bbox + haversine sobre `Profile`, reusa `match_engine`). **Frontend**: `StickerFilters` ampliado; `CatalogFilters` con `<select>` de disponibilidad + checkbox "Disponibles cerca" (+ radio 10/25/50/100 km, deshabilitado si no hay location, con hint); `/catalog/[slug]` pasa `userLocation` (de `useAuthStore.profile`, hace `refreshProfile()` al montar) + muestra `catalog-error` si un filtro requiere auth. Flow `catalog-availability-proximity` (flow-definitions v2.9.0, USER_FLOW_MAP v2.9.0, flow-tags). Tests: backend `test_sticker_filters.py` (11+2), frontend `CatalogFilters` (8) + `albumStore` (+1), e2e `availability-proximity.spec.ts` (2).
- ✅ F5 (commit pendiente): cobertura backend. **Hallazgo**: la cobertura global ya estaba en **~93%** (los tests añadidos en Bloques D/E/F la subieron desde el ~57% histórico) — los módulos más bajos están en 86–90%, no en 20-30% como decía el reporte viejo. Se hizo **1 tanda** (13 tests, no 2 — diminishing returns más allá de esto) cubriendo las ramas de error obvias de los archivos más bajos: `views/collectors.py` (invalid_limit/album_id/geo_params), `views/album.py` (`nearby` con lat/lng o radius_km no numéricos → 400), `services/qr_token.py` (tokens bien firmados con payload no-UTF8 / sin `|` / user_id≤0 + halves no-b64), `services/whatsapp_link.py` (trade unidireccional → "—"), `services/google_account_age.py` (createTime malformado, body no-JSON, earliest-de-varias-sources). Sin nuevos archivos de test.

**Bloque F: ✅ las 5 fases hechas.**
