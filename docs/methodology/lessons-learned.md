...
## 17. Bloque F — limpieza + hardening post-Bloque E

### El carryover del template incluye un sistema de auth completo que el producto no usa
- El `base_django_react_next_feature` traía auth email/password (sign_up/sign_in), reset por código (send_passcode/verify_passcode_and_reset_password/update_password + modelo `PasswordCode` + `email_service.py`), CRUD de usuarios (`user_crud.py` + 3 serializers) y un `/backoffice` dashboard que pegaba a un endpoint `sales/` inexistente — todo muerto, porque el producto es **solo Google OAuth** (las páginas `/sign-in`/`/sign-up` se reescribieron a Google-only en `c738634`). En Bloque F se podó todo eso: `views/auth.py` queda con `google_login` + `validate_token`; -1 view module, -1 url module, -3 serializers, -1 service, -1 model, +1 migración (`DeleteModel PasswordCode` — la tabla estaba vacía en dev/staging así que segura). Lección: cuando un template trae un subsistema entero, decidir pronto si el producto lo usa; si no, borrarlo de raíz (no solo "no enlazarlo") — si no, sigue pesando en la cobertura, el `tsc`, los conteos y la confusión.

### Trabajo I/O-bound dentro de un signal → mover a un `@db_task()` de Huey
- `signals.notify_on_mutual_match` llamaba `push_notify.send_to` (HTTP a los push services) **dentro** del `post_save Match` → bloqueaba el request del swipe. Ahora encola `deliver_match_push(user_id, payload)` (`@db_task()` en `albunmania_project/tasks.py`). El payload se pasa por valor (un dict serializable), no el objeto `User`; la task re-busca el `User` por id y hace un no-op si no existe (la tarea puede correr más tarde que el request). `HUEY = RedisHuey(..., immediate=not IS_PRODUCTION)` → en dev/test corre síncrona in-process, así que **no hace falta un Huey consumer ni `globalSetup` en los tests**; el límite async simplemente no se observa. **Dónde mockear en tests:** la task hace `from albunmania_app.services.push_notify import send_to` *dentro del cuerpo* → parchear `albunmania_app.services.push_notify.send_to` (la fuente), no `albunmania_app.signals.push_send_to` (que ya no existe). Patrón reusable para cualquier otro side-effect de signal que sea I/O (emails, webhooks, recomputaciones caras).
