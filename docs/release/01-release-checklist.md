...
GAPS detectados (auditoría 2026-05-12): los **4 GAPS P2 están cerrados** ("Bloque D") — D1 ✅ (T&C/Privacidad/FAQ pages + componente FAQ + footer), D2 ✅ (página /profile/[id] + endpoint public-profile + "Editar mi cuenta"), D3 ✅ (centro de notificaciones in-app + modelo Notification + campana en el Header), D4 ✅ (modelo Report general + reportar usuarios/intercambios + 2ª cola en /admin/moderation). GAPS P3 que quedan (`<!-- NO IMPLEMENTADO -->`/`<!-- V2 -->`): "en línea ahora"/Live Badge/presencia, Mapa de Coleccionistas, Geolocalización por IP (GeoIP2), dropdown de búsqueda predictiva.
-->
